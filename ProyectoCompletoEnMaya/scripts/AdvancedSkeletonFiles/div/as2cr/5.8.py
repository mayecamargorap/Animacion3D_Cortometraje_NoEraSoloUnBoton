...
class_name = selectedAsset.get_class().get_name()
if class_name == 'ControlRigRuntimeAsset':
	# UE5.8+ uses ControlRigRuntimeAsset + ControlRigEditorAsset
	runtime_asset = selectedAsset
	blueprint = runtime_asset.get_editor_asset() # ControlRigEditorAsset
	hierarchy = runtime_asset.hierarchy
	hierarchy_controller = blueprint.get_hierarchy_controller()
	library = blueprint.get_local_function_library()
	library_controller = blueprint.get_or_create_controller(library)
	RigVMController = blueprint.get_controller() #UE5.8
	try:
		...
		pass # rig_graph_display_settings not available on ControlRigEditorAsset
	...
	raise Exception('Selected object is not a ControlRigBlueprint or ControlRigRuntimeAsset, you must select a ControlRig')
